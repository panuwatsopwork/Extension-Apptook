...
final class Apptook_DS_Public {
	...
	public function handle_frontend_register(): void {
		...
		/**
		 * ให้ปลั๊กอินอื่น (เช่น Apptook Member Records) ซิงก์ข้อมูลลูกค้าแยกจากตาราง user ของ WP
		 */
		do_action( 'apptook_ds_customer_registered', (int) $user_id );
	...
	private function register_error_message_for_code( string $code ): string {
		switch ( $code ) {
			case 'invalid_nonce':
				return __( 'เซสชันหมดอายุหรือข้อมูลไม่ถูกต้อง กรุณาลองสมัครใหม่', 'apptook-digital-store' );
			...
			default:
				return __( 'ไม่สามารถสร้างบัญชีได้ กรุณาตรวจสอบข้อมูลหรือลองใหม่', 'apptook-digital-store' );
		}
	}
	...
	private function render_login_modal( string $redirect_url, string $register_url ): string {
		...
		ob_start();
		?>
		<div class="apptook-st-modal-overlay" id="apptook-st-login-modal" role="dialog" aria-modal="true" aria-labelledby="apptook-st-login-title" aria-hidden="true">
			<div class="apptook-st-modal">
				...
				<div class="apptook-st-modal-body">
					<h2 id="apptook-st-login-title" class="apptook-st-modal-title"><?php esc_html_e( 'Welcome Back', 'apptook-digital-store' ); ?></h2>
					<p class="apptook-st-modal-lead"><?php esc_html_e( 'Log in to manage your subscriptions.', 'apptook-digital-store' ); ?></p>
					<form class="apptook-st-modal-form" method="post" action="<?php echo esc_url( $login_action ); ?>">
						...
						<div class="apptook-st-field">
							...
							<input class="apptook-st-input" type="password" name="pwd" id="apptook_st_user_pass" autocomplete="current-password" required placeholder="<?php esc_attr_e( 'Enter password', 'apptook-digital-store' ); ?>" />
						</div>
	...
	public function shortcode_register(): string {
		...
		<div class="apptook-st-register apptook-stitch apptook-ds">
			<div class="apptook-st-register-card">
				...
				<?php endif; ?>
				<p class="apptook-st-register-hint">
					<?php
					echo esc_html(
						sprintf(
							/* translators: %d: minimum password length */
							__( 'ตั้งรหัสผ่านของคุณที่นี่ (อย่างน้อย %d ตัวอักษร) — ระบบอาจส่งอีเมลแจ้งผู้ดูแลตามการตั้งค่า WordPress', 'apptook-digital-store' ),
							$min_pw
						)
					);
					?>
				...
				<form class="apptook-st-modal-form" method="post" action="<?php echo esc_url( $form_action ); ?>" autocomplete="off">
					...
					<div class="apptook-st-field">
						<label class="apptook-st-label" for="apptook_reg_user_login"><?php esc_html_e( 'Username', 'apptook-digital-store' ); ?></label>
						<input class="apptook-st-input" type="text" name="user_login" id="apptook_reg_user_login" value="<?php echo isset( $_GET['user_login'] ) ? esc_attr( sanitize_user( wp_unslash( (string) $_GET['user_login'] ), true ) ) : ''; ?>" required autocomplete="username" placeholder="<?php esc_attr_e( 'Enter username', 'apptook-digital-store' ); ?>" />
					</div>
					<div class="apptook-st-field">
						<label class="apptook-st-label" for="apptook_reg_user_email"><?php esc_html_e( 'Email', 'apptook-digital-store' ); ?></label>
						<input class="apptook-st-input" type="email" name="user_email" id="apptook_reg_user_email" required autocomplete="email" placeholder="<?php esc_attr_e( 'your@email.com', 'apptook-digital-store' ); ?>" />
					</div>
					<div class="apptook-st-field">
						<label class="apptook-st-label" for="apptook_reg_user_pass"><?php esc_html_e( 'Password', 'apptook-digital-store' ); ?></label>
						<input class="apptook-st-input" type="password" name="user_pass" id="apptook_reg_user_pass" required autocomplete="new-password" minlength="<?php echo esc_attr( (string) $min_pw ); ?>" placeholder="<?php esc_attr_e( 'Create a password', 'apptook-digital-store' ); ?>" />
					</div>
					<div class="apptook-st-field">
						<label class="apptook-st-label" for="apptook_reg_user_pass2"><?php esc_html_e( 'Confirm password', 'apptook-digital-store' ); ?></label>
						<input class="apptook-st-input" type="password" name="user_pass_confirm" id="apptook_reg_user_pass2" required autocomplete="new-password" minlength="<?php echo esc_attr( (string) $min_pw ); ?>" placeholder="<?php esc_attr_e( 'Repeat password', 'apptook-digital-store' ); ?>" />
					</div>
