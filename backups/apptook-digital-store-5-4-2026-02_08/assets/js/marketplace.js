	...
	ready(function () {
		document.querySelectorAll('.apptook-stitch').forEach(function (root) {
			...
			root.addEventListener('click', function (e) {
				var buyBtn = e.target.closest('.buy-btn[data-product-id]');
				if (!buyBtn) {
					return;
				}
				if (buyBtn.disabled) {
				...
				if (window.ApptookDS && typeof window.ApptookDS.openPurchaseModal === 'function') {
					window.ApptookDS.openPurchaseModal({
				...
				if (window.ApptookDS && typeof window.ApptookDS.startOrderFlow === 'function') {
					window.ApptookDS.startOrderFlow(buyBtn.getAttribute('data-product-id'), buyBtn);
				}
			});
		});
	});
