...
function handleRuntimeMessage(webview, message) {
  ...
  if (message.type === 'startApiWorker') {
    const nextSession = mergeHostSession(hostRuntimeState && hostRuntimeState.session, incomingSession) || hostRuntimeState.session;
    ...
    try {
      webview.postMessage({
        type: 'startApiWorkerStatus',
        status: 'loading',
        message: 'Starting API worker...'
      });
    } catch (_err) {
      // ignore status notification failures
    }

    postEnsureWorkerToRuntimeWebview('startApiWorker');
    sendHostRuntimeState(webview);
    return;
  }
