...
const ADMIN_TOKEN = "jakkaphonezero";
...
function setupTablesManual() {
  return setupTables({ token: ADMIN_TOKEN });
}
...
function apiLogin(payload) {
  ...
  const usageState = buildUsageStateMetadata_(sourceEntries, sourceState, resolvedSource.entry.sourceKey, usageDate);
  if (!usageState.ok) {
...
function loopNextKey(payload) {
  ...
  const usageState = buildUsageStateMetadata_(sourceEntries, sourceState, nextSource.entry.sourceKey, usageDate);
  if (!usageState.ok) {
    return { ok: false, message: usageState.message };
  }
...
function dashboardSync(payload) {
  ...
  const usageState = buildUsageStateMetadata_(sourceEntries, sourceState, activeSourceKey, usageDate);
  if (!usageState.ok) {
    return { ok: false, message: usageState.message };
  }
...
function guardAdmin(token) {
  if (normalizeCell_(token) !== ADMIN_TOKEN) {
...
function buildUsageStateMetadata_(entries, sourceState, currentSourceKey, usageDate) {
  let totalAssignedSourceCapacity = 0;
  let rawUsageTotal = 0;
  ...
  (entries || []).forEach(function(entry) {
    ...
    rawUsageTotal += Math.min(usageState.cappedConsumedRaw, capacity);
