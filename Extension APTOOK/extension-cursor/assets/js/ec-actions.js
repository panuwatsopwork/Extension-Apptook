...
window.ExtensionCursorModules.createEcActions = function createEcActions({ api, ui, renderers, state, $, elements, viewData, monitorEdit }) {
  const { tabs, panels, refreshButton, importButton, clearLicenceForm, saveKeyButton, generateKeyButton, resetKeyButton, assignButton, unassignButton, replaceButton, licenceList, monitorRows, selectKey, licenceCode, tokenCapacity, activeDays, importRemark, apptookKey, note, expiry, debugAvailableButton, debugAssignmentButton } = elements;
  ...
  function bindEvents() {
    ...
    unassignButton.on('click', function () {
      ui.pressEffect(unassignButton);
      const keyId = parseInt(selectKey.val(), 10) || 0;
      const ids = selectedLicenceIds();
      if (!keyId || !ids.length) { ui.setNotice('กรุณาเลือก key และ licence อย่างน้อย 1 รายการ', 'error'); return; }
      ajaxFeedback(api.post('extension_cursor_unassign_licences', { key_id: keyId, licence_ids: ids }), 'unassign licence สำเร็จ', function () { reloadDashboard(keyId).always(() => selectKey.val(String(keyId))); });
    });

    replaceButton.on('click', function () {
      ui.pressEffect(replaceButton);
      const keyId = parseInt(selectKey.val(), 10) || 0;
      const ids = selectedLicenceIds();
      if (!keyId || !ids.length) { ui.setNotice('กรุณาเลือก key และ licence อย่างน้อย 1 รายการ', 'error'); return; }
      ajaxFeedback(api.post('extension_cursor_replace_licences', { key_id: keyId, licence_ids: ids }), 'replace licence สำเร็จ', function () { reloadDashboard(keyId).always(() => selectKey.val(String(keyId))); });
    });
    ...
    $(document).on('click', '.ec-monitor-key', function (event) {
      event.preventDefault();
      event.stopPropagation();
      ...
      $.post(ExtensionCursorAdmin.ajaxUrl, { action: 'extension_cursor_dashboard_snapshot', nonce: ExtensionCursorAdmin.nonce, key_id: keyId })
        .done(function (response) {
          if (!response || !response.success || !response.data) return;
          const monitor = response.data.monitor || {};
          renderers.renderMonitorDetail(monitor);
        });
    });
